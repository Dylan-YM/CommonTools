...
#define LogC(flg, frmt, ...)    LOG_MAYBE(LogAsync, logLevel, flg, LogContext, nil, __PRETTY_FUNCTION__, frmt, ##__VA_ARGS__)
...
#define LogCError(frmt, ...)    LogC(DDLogFlagError,   (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)
#define LogCWarn(frmt, ...)     LogC(DDLogFlagWarning,    (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)
#define LogCInfo(frmt, ...)     LogC(DDLogFlagInfo,    (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)
#define LogCVerbose(frmt, ...)  LogC(DDLogFlagVerbose, (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)

#define LogTrace()              LogObjc(DDLogFlagVerbose, @"%@: %@", THIS_FILE, THIS_METHOD)
#define LogCTrace()             LogC(DDLogFlagVerbose, @"%@: %s", THIS_FILE, __FUNCTION__)
...
+ (NSMutableArray *)lookupHost:(NSString *)host port:(uint16_t)port error:(NSError **)errPtr
{
	...
	else
	{
		...
		hints.ai_protocol = IPPROTO_TCP;//IPPROTO_TCP
